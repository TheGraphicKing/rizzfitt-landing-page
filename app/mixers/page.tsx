...
import { Activity, RefreshCw, Users, Coffee, Heart, ArrowRight, type LucideIcon } from "lucide-react";
...
export const metadata: Metadata = {
  title: "Social Mixers — meet people through movement",
  description:
    "Beginner-first pickleball mixers where a game is the icebreaker and breakfast is the reason to stay. Come alone, leave with a group chat.",
};
...
export default function MixersPage() {
  ...
  const featured = mixers[0];
  const reserve = featured ? canonicalEventUrl(featured) : { href: "/events", external: false };
  ...
  return (
    <>
      ...
      <section className="event-hero" data-mode="live">
        ...
        <div className="container event-hero-inner">
          <div className="stack" style={{ gap: "var(--space-5)", maxWidth: "44rem" }}>
            ...
            <p className="body-l">
              Beginner-first pickleball mixers where a game is the icebreaker and
              breakfast is the reason to stay. Come alone, leave with a group chat.
              Monthly, and unmistakably Bangalore (for now).
            </p>
            <div className="cluster">
              <Button href={reserve.href} variant="primary" external={reserve.external}>
                Reserve a spot
              </Button>
      ...
      {mixers.length ? (
        <Section mode="live">
          ...
          >
            ...
            <Link href="/events" className="cluster" style={{ gap: 6, color: "var(--accent)", fontWeight: 600 }}>
      ...
      <Section mode="live">
        <CTABand title="Find your Bangalore crowd." primary={{ label: "Reserve a spot", href: reserve.href, external: reserve.external }} />
      </Section>
    </>
  );
}
